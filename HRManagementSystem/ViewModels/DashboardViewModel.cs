...
public class DashboardViewModel : ViewModelBase
{
    ...
    public DashboardViewModel(Window? parentWindow = null)
    {
        ...
        // 初始化命令
        NewAnnouncementCommand = new DelegateCommand(OnNewAnnouncement);
        NewTodoCommand = new DelegateCommand(OnNewTodo);
        NewEmployeeCommand = new DelegateCommand(OnNewEmployee);
        NewDepartmentCommand = new DelegateCommand(OnNewDepartment);
        NewPositionCommand = new DelegateCommand(OnNewPosition);
        NewAttendanceCommand = new DelegateCommand(OnNewAttendance);
        NewSalaryCommand = new DelegateCommand(OnNewSalary);
        // 加载模拟数据
        LoadDemoData();
    }
        ...
        Dispatcher.UIThread.Post(async () =>
        {
            try
            {
                ...
                if (owner != null)
                {
                    ...
                    if (result != null) Announcements.Insert(0, result);
                }
                else
                {
            ...
            {
                // 记录异常信息但不抛出
                Debug.WriteLine($"创建公告时出错: {ex.Message}");
            }
        });
    }
    ...
    private void OnNewTodo()
    {
        ...
        Dispatcher.UIThread.Post(async () =>
        {
            ...
            {
                // 记录异常信息但不抛出
                Debug.WriteLine($"创建待办事项时出错: {ex.Message}");
            }
        });
    }
    ...
    private void LoadDemoData()
    {
        ...
        {
            new() { Task = "审核本月员工考勤记录", IsCompleted = true },
            new() { Task = "准备下月部门预算报告", IsCompleted = false },
            new() { Task = "与IT部门讨论系统升级事宜", IsCompleted = false },
            new() { Task = "安排新员工入职培训", IsCompleted = false },
            new() { Task = "更新公司员工手册", IsCompleted = false }
        };

        // 加载部门分布数据
        DepartmentDistribution = new ObservableCollection<DepartmentDistributionItem>
        {
            new() { Name = "技术部", Count = 42, Color = SolidColorBrush.Parse("#2196F3") },
            new() { Name = "市场部", Count = 28, Color = SolidColorBrush.Parse("#4CAF50") },
            new() { Name = "销售部", Count = 18, Color = SolidColorBrush.Parse("#FFC107") },
            new() { Name = "人事部", Count = 12, Color = SolidColorBrush.Parse("#9C27B0") },
            new() { Name = "财务部", Count = 10, Color = SolidColorBrush.Parse("#F44336") },
            new() { Name = "行政部", Count = 8, Color = SolidColorBrush.Parse("#FF9800") },
            new() { Name = "客服部", Count = 6, Color = SolidColorBrush.Parse("#607D8B") },
            new() { Name = "其他", Count = 4, Color = SolidColorBrush.Parse("#9E9E9E") }
        };
    }
}
